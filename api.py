...
def configure_ngrok():
    ngrok_token = "2n3XSAm9R8ehwI4XclIPU0o3uH6_7W81pQMp23cvzPKnowMff"
    try:
...
def update_soul_txt(public_url):
    with open("mahakal6.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in mahakal6.txt")

def update_vps_soul_txt(public_url):
    vps_ip = "94.131.9.251"
    vps_user = "root"
    vps_password = "ldQ07YS2y0Dz"
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("mahakal6.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated mahakal6.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update mahakal6.txt on VPS: {str(e)}")
